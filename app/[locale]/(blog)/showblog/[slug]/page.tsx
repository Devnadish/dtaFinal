...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generateSEO({
    title: post.seo?.metaTitle ?? post.title ?? 'Default Title',
    description: post.seo?.metaDescription ?? post.description ?? 'Default Description',
    image: post.mainImage,
    keywords: post.seo?.keywords ?? [],
    // author: post.author?.name ?? siteConfig.author ?? 'Default Author',
...
export default async function Page({ params }: { params: Promise<{ locale: string, slug: string }> }) {
  ...
  const structuredData = generateStructuredData({
    ...
    image: post.mainImage,
    // author: post.author?.name || "khalid nadish",
    publishedAt: post.publishedAt,
    // modifiedAt: post.seo?.dateModified,
    locale,
    slug: `showblog/${slug}`,
          ...
          {post.categories && (
            <div className="flex flex-wrap gap-2 mb-8">
              {/* {post.categories.map((category) => (
                <Text variant="h1" locale={locale} key={category.title} className="px-3 py-1 text-sm rounded-full bg-primary/10 text-primary">
                  {category.title ??}
                </Text>
              ))} */}
            </div>
